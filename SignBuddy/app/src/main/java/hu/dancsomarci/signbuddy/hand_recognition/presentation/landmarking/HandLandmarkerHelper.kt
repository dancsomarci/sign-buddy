...
class HandLandmarkerHelper(
    ...
    val onError: (error: String, errorCode: Int) -> Unit,
    val onResults: (resultBundle: ResultBundle) -> Unit
) {
...
//                }
                        ...
                        ?.let { detectionResult ->
                            ...
                            onError(
                                "ResultBundle could not be returned" +
                                        " in detectVideoFile", OTHER_ERROR
                            )
                        }
                }
                ?: run {
                    ...
                    onError(
                        "Frame at specified time could not be" +
                                " retrieved when detecting in video.", OTHER_ERROR
                    )
                }
        }
    ...
    fun detectImage(image: Bitmap): ResultBundle? {
        ...
        // to indicate this.
        onError(
            "Hand Landmarker failed to detect.", OTHER_ERROR
        )
        return null
    }
    ...
    private fun returnLivestreamError(error: RuntimeException) {
        onError(
            error.message ?: "An unknown error has occurred", OTHER_ERROR
        )
    }
